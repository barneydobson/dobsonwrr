...
int main(int argc, char* argv[])
{
	...
	int numFramingsForEval = 4, numFramings = 8;
	int coopIndex[8] = {0,0,0,0,1,1,1,1};
	int evalIndex[8] = {0,0,0,0,1,1,1,1};
	int framingSpecifier[8] = {1,2,3,4,5,6,7,8};
	...
	int objIndex[4] = {0,2,1,3}; //Same as above - not a very elegant solution (note this index describes how placing wes objs next to sww objs relates to the order of coop objs)
	int revalT = 20000; 
	int enSize = 500;
	...
	dummyEnsemble.percentile = 50;
	dummyEnsemble.simulationType = 0;
	createEnsemble(&dummyEnsemble,1);
	
	char *fid = calloc(256,sizeof(char));
	sprintf(fid,"reval_ensembleSize%d_T%d.reeval",enSize,revalT);; //Consider making this .runtime - also consider a more informative filename	
	DIR *dir;
	struct dirent *ent;
	char *fStr = "optimization_framing";
			...
			while ((ent = readdir (dir)) != NULL) 
			{
				...
				if (currentFrame == framingSpecifier[k])
				{
					if (coopIndex[k] == 1)
					{
						...
						tempEnsemble.ensembleSize = enSize;
						tempEnsemble.ensembleT = revalT;
						tempEnsemble.baseSeed = 12345;
						tempEnsemble.percentile = 50;
						...
						while (temp != NULL)
						{
							if (i%size == rank)
							{
								evaluateEnsemble(temp->weights,temp->objectives,0,&tempEnsemble);
								if (rank != 0)
								{
					...
					else
					{
						...
						switch (currentSimType){
							case 1:
								...
								tempC2Ensemble.baseSeed = 12345;
								tempC2Ensemble.percentile = 50;
								...
								while (c2Temp != NULL)
								{
									if (i%size == rank)
									{
										evaluateEnsemble(c2Temp->weights,c2Temp->objectives,0,&tempC2Ensemble);
										if (rank != 0)
										{
							...
							case 2:
								...
								tempC1Ensemble.baseSeed = 12345;
								tempC1Ensemble.percentile = 50;
								...
								while (c1Temp != NULL)
								{
									if (i%size == rank)
									{
										evaluateEnsemble(c1Temp->weights,c1Temp->objectives,0,&tempC1Ensemble);
										if (rank != 0)
										{
	...
	for (l=0; l < numFramings; l++)
	{
		...
		if (evalIndex[l] == 1)
		{
			...
			{	
				...
				myEnsemble.baseSeed = k+l*numFramings + 1;
				...
				// if (k + 1 <4)
				// {
					// printf("weights for %d\n",k+1);
					// printWeights(p,(26+28));
				...
				while (p != NULL){
					...
					if (i%size == rank)
					{
						evaluateEnsemble(p->weights,objectives,0,&myEnsemble);
						if (rank != 0)
						{
